...
## 11. Security audit preflight

Before production rollout of kernel-cell changes, run this preflight set:

```sh
# build check (in source tree)
nbmake -C sys/modules/secmodel_cell dependall all

# runtime contract checks (on target NetBSD host)
...
Then complete `doc/specs/cells/06-kernel-audit-checklist.md` and attach the
command transcript to the change review.

...
int split_shell_words(const char *input, char ***argv_out, int *argc_out,
                      char **err_out) {
  char **argv = NULL;
  int argc = 0;
  int cap = 0;
  ...
  *err_out = NULL;
  ...
  for (const char *p = input;; p++) {
    ...
    if (escaped && !at_end) {
      if (curr_len + 1 >= curr_cap) {
        size_t next_cap = (curr_cap == 0) ? 32 : curr_cap * 2;
        char *next = realloc(current, next_cap);
        if (next == NULL) {
    ...
    if (at_end) {
      if (escaped) {
        *err_out = xstrdup("unexpected end after escape");
        goto fail;
      }
      if (quote != '\0') {
        *err_out = xstrdup("unclosed quote");
        goto fail;
      }
      if (curr_len > 0 || had_quoted) {
        if (curr_len + 1 >= curr_cap) {
          char *next = realloc(current, curr_len + 1);
          if (next == NULL) {
            *err_out = xstrdup("out of memory");
            goto fail;
          }
        ...
        if (argc == cap) {
          ...
          char **next_argv =
              realloc(argv, (size_t)next_cap * sizeof(*next_argv));
          if (next_argv == NULL) {
            *err_out = xstrdup("out of memory");
            goto fail;
          }
